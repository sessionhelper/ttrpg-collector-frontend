...
import { Ghost, Flag, Pencil, UserRoundX, Volume2 } from "lucide-react";
...
const SPEAKER_NAMES: Record<string, string> = {
  "102597e0ee793a7b": "Marcus",
  "2f09cc7c1965a203": "GM",
  "787a9547c27cd7f8": "Riza",
  "d0143e57ce524cdc": "Holt",
};

const GM_ID = "2f09cc7c1965a203";

// Speaker accent colors — used for the left bar
const SPEAKER_ACCENTS: Record<string, { light: string; dark: string }> = {
  "102597e0ee793a7b": { light: "#2563eb", dark: "#60a5fa" },
  "2f09cc7c1965a203": { light: "#8b4513", dark: "#d4a574" },
  "787a9547c27cd7f8": { light: "#15803d", dark: "#4ade80" },
  "d0143e57ce524cdc": { light: "#7c3aed", dark: "#a78bfa" },
};

function getSpeakerAccent(id: string, dark: boolean): string {
  const colors = SPEAKER_ACCENTS[id];
  if (!colors) return dark ? "#666" : "#999";
  return dark ? colors.dark : colors.light;
}

function speakerName(id: string): string {
  return SPEAKER_NAMES[id] ?? id.substring(0, 8);
}
...
function buildBlocks(segments: Segment[]): Block[] {
  ...
  while (i < segments.length) {
    ...
    if (speakerSet.size > 1) {
      ...
      const hasGM = speakersInBatch.includes(GM_ID);
      if (hasGM) {
        const other = speakersInBatch.find((s) => s !== GM_ID) ?? speakersInBatch[0];
        columnCtx = [GM_ID, other];
      } else {
...
export default function PrototypePage() {
  ...
  useEffect(() => {
    fetch("/transcript.json").then((r) => r.json()).then((d: PipelineResult) => setData(d));
  }, []);
    ...
    <div className="min-h-screen transition-colors duration-200" style={{ background: t.bg }}>
      ...
      <main className="max-w-[620px] mx-auto pl-14 pr-12 py-6">
        ...
        <div className="flex gap-4 mb-6 font-sans text-xs" style={{ color: t.textMuted }}>
          {Object.entries(SPEAKER_NAMES).map(([id, name]) => (
        ...
        {blocks.map((block, bi) => {
          ...
          const isGM = block.type === "single" && block.segments[0]?.speaker_pseudo_id === GM_ID;
